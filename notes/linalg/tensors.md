...
This means that if $S$ is finite, then $F(S)$ is finite dimensional, with $dim F(S) = |S|$. If $S$ isn't finite, then it has an infinite basis, and we have defined dimension yet for "infinite-dimensional" vector spaces.

### Thinking about $F(U \times V)$
What are the elements of $F(U \times V)$? They are functions that assign scalars to finitely many pairs in $U \times V$, in essence. In other words, they are linear combinations of elements of $U \times V$, with the catch that no pair $(u, v)$ is a linear combination of pairs that are distinct from $(u, v)$. This is certainly different from from the direct product space $U \times V$: if you take arbitrary finite sets of pairs, many sets will be dependent. If $U = V = \mathbb{R}$, for example, then $U  TODO


### An alternative definition of tensor product
We are going to be defining the tensor product of finite-dimensional spaces $U$ and $V$ as a subspace of the free vector space generated by $U \times V$. Note that in Halmos' definition, the tensor product $U \otimes V$ was finite dimensional, with $dim U \otimes V = dim U dim V$. But $F(U \times V)$ is not, in general, a finite-dimensional vector space, since the set $U \times V$ is an infinite set for many of the fields that we care a great deal about, like $\mathbb{R}$ and $\mathbb{C}$. This is one reason why we must find a suitable finite-dimensional subspace of $F(U \times V)$ if we want the dimensions to match for this alternate definition of tensor products.
